...
class Model extends BaseModel
{
    ...
    protected function doUpdate($id = null, $row = null): bool
    {
        ...
        log_message('debug', 'doUpdate called with id: ' . json_encode($id) . ', row: ' . json_encode($row));
        
        // Reset escape array to prevent corruption
        $this->escape = [];
        
        $builder = $this->builder();

        // Handle case where $id is an array (from BaseModel::update)
        if (is_array($id) && !empty($id)) {
            $builder = $builder->whereIn($this->table . '.' . $this->primaryKey, $id);
        } elseif (! in_array($id, [null, '', 0, '0', []], true)) {
        ...
        // Use update() method directly instead of set() to avoid escape array issues
